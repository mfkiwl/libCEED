        ...
        try:
            ...
            if not np.allclose(y0_database_value, y0_correct_value, atol=atol, rtol=rtol):
                ...
                if not np.all(idx_notclose[1] == 4):
                    ...
                    test_fail = False if np.allclose(-database_vorticity, correct_vorticity, atol=atol, rtol=rtol) else True

                if test_fail:
                    database_output_path = Path(f"./y0_database_values_{ceed_resource.replace('/', '_')}.npy").absolute()
