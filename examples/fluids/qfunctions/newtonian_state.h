...
CEED_QFUNCTION_HELPER void UnpackState_V(StateEntropy s, CeedScalar V[5]) {
  V[0] = s.chemical_potential;
  for (int i = 0; i < 3; i++) V[i + 1] = s.velocity[i];
  V[4] = s.temperature;
}
...
CEED_QFUNCTION_HELPER StateEntropy StateEntropyFromConservative(NewtonianIdealGasContext gas, StateConservative U, const CeedScalar x[3]) {
  ...
  V.chemical_potential = (gamma - entropy) / (gamma - 1) - (E_kinetic + E_potential) / P;
  for (int i = 0; i < 3; i++) V.velocity[i] = U.momentum[i] / P;
  V.temperature = -U.density / P;
  return V;
}
...
CEED_QFUNCTION_HELPER StateEntropy StateEntropyFromConservative_fwd(NewtonianIdealGasContext gas, State s, StateConservative dU,
  ...
  dV.chemical_potential = -dentropy / (gamma - 1) - (dE_kinetic + dE_potential) / P + dP * (E_kinetic + E_potential) / Square(P);
  for (CeedInt i = 0; i < 3; i++) {
    dV.velocity[i] = (dU.momentum[i] - s.U.momentum[i] * dP / P) / P;
  }
  dV.temperature = -(dU.density - s.U.density * dP / P) / P;
  return dV;
}
...
CEED_QFUNCTION_HELPER StateConservative StateConservativeFromEntropy(NewtonianIdealGasContext gas, StateEntropy V, const CeedScalar x[3]) {
  ...
  for (int i = 0; i < 3; i++) velocity[i] = -V.velocity[i] / V.temperature;
  const CeedScalar gamma       = HeatCapacityRatio(gas);
  const CeedScalar e_kinetic   = 0.5 * Dot3(velocity, velocity);
  const CeedScalar e_potential = -Dot3(gas->g, x);
  const CeedScalar entropy     = gamma - (gamma - 1) * (V.chemical_potential - (e_kinetic + e_potential) * V.temperature);
  const CeedScalar log_rho     = -(entropy + log(-V.temperature)) / (gamma - 1);
  U.density                    = exp(log_rho);
  for (int i = 0; i < 3; i++) U.momentum[i] = U.density * velocity[i];

  const CeedScalar e_internal = -gas->cv / (GasConstant(gas) * V.temperature);
...
CEED_QFUNCTION_HELPER StateConservative StateConservativeFromEntropy_fwd(NewtonianIdealGasContext gas, State s, StateEntropy dV,
  ...
  for (int i = 0; i < 3; i++) dvelocity[i] = (-dV.velocity[i] - s.Y.velocity[i] * dV.temperature) / s.V.temperature;
  const CeedScalar gamma        = HeatCapacityRatio(gas);
  ...
  const CeedScalar entropy      = gamma - (gamma - 1) * (s.V.chemical_potential - (e_kinetic + e_potential) * s.V.temperature);
  const CeedScalar dentropy =
      -(gamma - 1) * (dV.chemical_potential - ((de_kinetic + de_potential) * s.V.temperature + (e_kinetic + e_potential) * dV.temperature));
  const CeedScalar log_rho = -(entropy + log(-s.V.temperature)) / (gamma - 1);
  const CeedScalar rho     = exp(log_rho);
  dU.density               = -rho / (gamma - 1) * (dentropy + dV.temperature / s.V.temperature);
  for (int i = 0; i < 3; i++) dU.momentum[i] = dU.density * s.Y.velocity[i] + s.U.density * dvelocity[i];

  const CeedScalar e_internal  = -gas->cv / (GasConstant(gas) * s.V.temperature);
  const CeedScalar de_internal = gas->cv * dV.temperature / (GasConstant(gas) * s.V.temperature * s.V.temperature);
...
CEED_QFUNCTION_HELPER StateEntropy StateEntropyFromPrimitive(NewtonianIdealGasContext gas, StatePrimitive Y, const CeedScalar x[3]) {
  ...
  V.chemical_potential = (gamma - entropy) / (gamma - 1) - rho_div_p * (e_kinetic + e_potential);
  for (int i = 0; i < 3; i++) V.velocity[i] = rho_div_p * Y.velocity[i];
  V.temperature = -rho_div_p;
  return V;
}
...
CEED_QFUNCTION_HELPER StateEntropy StateEntropyFromPrimitive_fwd(NewtonianIdealGasContext gas, State s, StatePrimitive dY, const CeedScalar x[3],
  ...
  dV.chemical_potential = -dentropy / (gamma - 1) - (de_kinetic + de_potential) * rho_div_p - (e_kinetic + e_potential) * drho_div_p;
  for (CeedInt i = 0; i < 3; i++) dV.velocity[i] = rho_div_p * dY.velocity[i] + drho_div_p * s.Y.velocity[i];
  dV.temperature = -drho_div_p;
  return dV;
}
...
CEED_QFUNCTION_HELPER StatePrimitive StatePrimitiveFromEntropy(NewtonianIdealGasContext gas, StateEntropy V, const CeedScalar x[3]) {
  StatePrimitive Y;
  for (int i = 0; i < 3; i++) Y.velocity[i] = -V.velocity[i] / V.temperature;
  Y.temperature                = -1 / (GasConstant(gas) * V.temperature);
  const CeedScalar gamma       = HeatCapacityRatio(gas);
  const CeedScalar e_kinetic   = 0.5 * Dot3(Y.velocity, Y.velocity);
  const CeedScalar e_potential = -Dot3(gas->g, x);
  const CeedScalar entropy     = gamma - (gamma - 1) * (V.chemical_potential - (e_kinetic + e_potential) * V.temperature);
  const CeedScalar log_P       = -(entropy + gamma * log(-V.temperature)) / (gamma - 1);
...
CEED_QFUNCTION_HELPER StatePrimitive StatePrimitiveFromEntropy_fwd(NewtonianIdealGasContext gas, State s, StateEntropy dV, const CeedScalar x[3],
                                                                   const CeedScalar dx[3]) {
  StatePrimitive dY;
  for (int i = 0; i < 3; i++) dY.velocity[i] = -(dV.velocity[i] - s.V.velocity[i] * dV.temperature / s.V.temperature) / s.V.temperature;
  dY.temperature                = dV.temperature / (GasConstant(gas) * s.V.temperature * s.V.temperature);
  const CeedScalar gamma        = HeatCapacityRatio(gas);
  const CeedScalar e_kinetic    = 0.5 * Dot3(s.Y.velocity, s.Y.velocity);
  const CeedScalar de_kinetic   = Dot3(dY.velocity, s.Y.velocity);
  const CeedScalar e_potential  = -Dot3(gas->g, x);
  const CeedScalar de_potential = -Dot3(gas->g, dx);
  const CeedScalar dentropy =
      (1 - gamma) * (dV.chemical_potential - (e_kinetic + e_potential) * dV.temperature - (de_kinetic + de_potential) * s.V.temperature);
  dY.pressure = s.Y.pressure * (-dentropy - gamma * dV.temperature / s.V.temperature) / (gamma - 1);
  return dY;
}
...
CEED_QFUNCTION_HELPER State StateFromV_fwd(NewtonianIdealGasContext gas, State s, const CeedScalar dV[5], const CeedScalar x[3],
                                           const CeedScalar dx[3]) {
  State ds;
  ds.V.chemical_potential = dV[0];
  ds.V.velocity[0]        = dV[1];
  ds.V.velocity[1]        = dV[2];
  ds.V.velocity[2]        = dV[3];
  ds.V.temperature        = dV[4];
