...
int main(int argc, char **argv) {
  ...
  if (read_mesh) {
    ...
    PetscCall(DMPlexCreateBoxMesh(PETSC_COMM_WORLD, dim, PETSC_FALSE, mesh_elem, NULL, NULL, NULL, PETSC_TRUE, &dm));
  }
  ...
  PetscCall(SetupLibceedByDegree(dm, ceed, degree, dim, q_extra, dim, num_comp_u, g_size, xl_size, bp_options[bp_choice], ceed_data, true, rhs_ceed,
                                 &target));
